...
- For repeated takes, prefer delete-before/keep-after when quality is close. Later takes are usually corrections; keep an earlier take only when the later take is clearly lower confidence, incomplete, or semantically wrong. This applies to obvious repeated spoken content even when the earlier take was already aligned to the reference.
...
- Per-line browser playback is a source-timeline listening tool. The line `播放` button and `Cmd/Ctrl+P` must play that line's original source range exactly and must not use edited-timeline skip logic, hidden pre-roll, or post-roll that makes adjacent lines audible.
- The `删线音频` player is the edited-result preview. It should play the same source/proxy media while automatically jumping over deleted intervals, equivalent to listening to the final kept audio, not muting deleted audio in place.
...
7. In the browser, review only the AI polish focus items, EDL/clustering conflicts, and any pending low-confidence audio questions. Use the `重点`, `ASR空窗`, `密集数字`, `术语风险`, and `删除建议` filters and colored chips to prioritize listening; do not hunt through raw gray flag text. Use `文案候选` for reference-unit review and `全文行` for line-level cleanup. Per-line play must use the exact source-time line range. The `删线音频` player is the continuous final-audio preview and should auto-skip deleted intervals. `Command+Z` / `Ctrl+Z` undo current-session edits; `Command+Shift+Z` / `Ctrl+Shift+Z` redo.
