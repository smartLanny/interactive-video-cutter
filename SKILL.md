...
- ASR context policy is provider-specific. Local Qwen first pass keeps global reference context off by default because long chunked decoding can be biased or truncated; use `--asr-context-file` or `create_review_project.py --asr-context` only for explicit local experiments or targeted dense technical ranges. Volcengine Seed ASR 2.0 standard A/B runs enable reference-derived context by default unless `--no-asr-context` is passed.
- Volcengine Seed ASR 2.0 standard is an optional cloud comparison provider, not the default local workflow. Use `VOLCENGINE_ASR_API_KEY` from the environment, `--provider volcengine`, and direct whole-file `audio.data` upload by default; pass `--volcengine-audio-url` or `--volcengine-no-data-upload` only when URL mode is required. Never commit or print API keys.
...
2. Run `create_review_project.py --media <audio-or-video> --reference <script.md> --workdir <review-workdir>`. For video, this first writes `<workdir>/edit/audio/<media-stem>_asr.m4a`; the review page uses this audio for human second-pass segmentation and correction. The default local Qwen pass uses no global context so the first transcript stays closest to the audio. Use `--asr-context` only for explicit local reference-derived context experiments, and use `--asr-context-file` for targeted local reruns. For cloud A/B, run `--provider volcengine`; it uploads the whole selected audio file, enables reference-derived context by default, and writes the same transcript JSON shape. Reference-script projects default to `--preprocess-mode reference-grouped`; that mode aligns reference semantic units to ASR words in source order before display-line splitting. Use `--preprocess-mode flat` only when you want the older line list behavior.
